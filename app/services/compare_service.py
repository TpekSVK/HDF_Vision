...
def _align_by_pose(golden: np.ndarray, img_src: np.ndarray, mask_pose: Optional[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
    return _ecc_multiscale(golden, img_src, mask_pose)
...
def analyze(
    ...
    # 3) SSIM v ROI (GPU ak je, inak CPU)
    ...
    # aplikuj ROI masku a uprac morfológiou
