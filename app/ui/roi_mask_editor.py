...
class _CircleCommit:
    ...
    fill_mode: str
    thickness: int
    mask_base: Optional[np.ndarray]


@dataclass
class _RectangleCommit:
    rect: QRectF
    fill_mode: str
    thickness: int
    mask_base: Optional[np.ndarray]
...
class _MaskView(_ImageView):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._rectangle_preview_rect: Optional[QRectF] = None
        self._circle_preview: Optional[Tuple[QPointF, float]] = None
        self._shape_dragging = False
        self._shape_drag_last_pos: Optional[QPointF] = None
        self._active_circle_edit: Optional[_CircleCommit] = None
        self._last_circle_commit: Optional[_CircleCommit] = None
        self._active_rectangle_edit: Optional[_RectangleCommit] = None
        self._last_rectangle_commit: Optional[_RectangleCommit] = None
    ...
    def set_pixmap(self, pixmap: Optional[QPixmap]) -> None:  # type: ignore[override]
        ...
        self._rectangle_preview_rect = None
        self._circle_preview = None
        self._shape_dragging = False
        self._shape_drag_last_pos = None
        self._active_circle_edit = None
        self._last_circle_commit = None
        self._active_rectangle_edit = None
        self._last_rectangle_commit = None
    ...
    def set_mask(self, mask: Optional[np.ndarray]) -> None:
        if mask is None:
            ...
            self._shape_dragging = False
            self._shape_drag_last_pos = None
            self._reset_shape_commit_history()
            self._update_mask_item()
            self._update_roi_overlay()
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self._mode == self.MODE_SHAPE_CIRCLE:
                if self._circle_preview is not None:
                    center, radius = self._circle_preview
                    dx = scene_pos.x() - center.x()
                    dy = scene_pos.y() - center.y()
                    allow_drag = False
                    if self._active_circle_edit is not None:
                        allow_drag = self._point_in_circle_commit(
                            self._active_circle_edit, scene_pos
                        )
                    else:
                        allow_drag = radius > 0 and math.hypot(dx, dy) <= radius
                    if allow_drag:
                        self._shape_dragging = True
                        self._shape_drag_last_pos = scene_pos
                        event.accept()
                        return
                    self._circle_preview = None
                    self._remove_shape_preview()
                    ...
                    self._last_circle_commit is not None
                    and self._point_in_circle_commit(self._last_circle_commit, scene_pos)
                ):
                    self._begin_circle_edit(self._last_circle_commit, scene_pos)
                    event.accept()
                    return
                self._reset_shape_commit_history()
                self._circle_points.clear()
                ...
                return
            if self._mode == self.MODE_SHAPE_RECTANGLE:
                if self._rectangle_preview_rect is not None:
                    allow_drag = False
                    if self._active_rectangle_edit is not None:
                        allow_drag = self._point_in_rectangle_commit(
                            self._active_rectangle_edit, scene_pos
                        )
                    else:
                        allow_drag = self._rectangle_preview_rect.contains(scene_pos)
                    if allow_drag:
                        self._shape_dragging = True
                        self._shape_drag_last_pos = scene_pos
                        event.accept()
                        return
                if (
                    self._last_rectangle_commit is not None
                    and self._point_in_rectangle_commit(
                        self._last_rectangle_commit, scene_pos
                    )
                ):
                    self._begin_rectangle_edit(self._last_rectangle_commit, scene_pos)
                    event.accept()
                    return
                self._rectangle_preview_rect = None
                self._remove_shape_preview()
    ...
    def mouseMoveEvent(self, event) -> None:  # noqa: N802 - Qt API
        ...
        if self._mode == self.MODE_SHAPE_CIRCLE:
            if (
                ...
                and self._circle_preview is not None
                and event.buttons() & Qt.LeftButton
            ):
                scene_pos = _clamp_point_to_rect(self.mapToScene(event.pos()), self.scene_rect())
                self._move_circle_preview(scene_pos)
                event.accept()
                return
            if self._circle_points:
        ...
        if self._mode == self.MODE_SHAPE_RECTANGLE:
            if (
                self._shape_dragging
                and self._rectangle_preview_rect is not None
                and event.buttons() & Qt.LeftButton
            ):
                scene_pos = _clamp_point_to_rect(self.mapToScene(event.pos()), self.scene_rect())
                self._move_rectangle_preview(scene_pos)
                event.accept()
                return
    ...
    def mouseReleaseEvent(self, event) -> None:  # noqa: N802 - Qt API
        if event.button() == Qt.LeftButton:
            ...
            if self._mode == self.MODE_SHAPE_RECTANGLE and self._shape_start is not None:
                ...
                self._shape_start = None
                rect = self._rectangle_rect_from_points(start, scene_pos)
                self._set_rectangle_preview_rect(rect, record_history=True)
                event.accept()
                return
    ...
    def mouseDoubleClickEvent(self, event) -> None:  # noqa: N802 - Qt API
        if event.button() == Qt.LeftButton:
            if (
                ...
                self._commit_rectangle_preview()
                event.accept()
                return
            if self._mode == self.MODE_SHAPE_CIRCLE and self._circle_preview is not None:
                self._commit_circle_preview()
                event.accept()
                return
    ...
    def keyPressEvent(self, event) -> None:  # noqa: N802 - Qt API
        ...
        if event.key() == Qt.Key_Escape:
            if self._mode == self.MODE_SHAPE_RECTANGLE and (
                self._shape_start is not None or self._rectangle_preview_rect is not None
            ):
                self._shape_start = None
                self._remove_shape_preview()
                self._shape_dragging = False
                ...
                event.accept()
                return
            if self._mode == self.MODE_SHAPE_CIRCLE and (
                self._circle_points or self._circle_preview is not None
            ):
                self._circle_points.clear()
                self._remove_shape_preview()
                self._shape_dragging = False
    ...
    def _clone_circle_commit(self, commit: _CircleCommit) -> _CircleCommit:
        return _CircleCommit(
            QPointF(commit.center),
            float(commit.radius),
            commit.fill_mode,
            int(commit.thickness),
            commit.mask_base.copy() if commit.mask_base is not None else None,
        )

    def _clone_rectangle_commit(self, commit: _RectangleCommit) -> _RectangleCommit:
        return _RectangleCommit(
            QRectF(commit.rect),
            commit.fill_mode,
            int(commit.thickness),
            commit.mask_base.copy() if commit.mask_base is not None else None,
    ...
    def _point_in_circle_commit(self, commit: _CircleCommit, point: QPointF) -> bool:
        dx = point.x() - commit.center.x()
        dy = point.y() - commit.center.y()
        distance = math.hypot(dx, dy)
        if commit.fill_mode == self.FILL_AROUND:
            outer = commit.radius + max(1.0, float(commit.thickness))
            inner = max(0.0, commit.radius - 2.0)
            return inner <= distance <= outer
        return distance <= commit.radius + 2.0

    def _point_in_rectangle_commit(
        self, commit: _RectangleCommit, point: QPointF
    ) -> bool:
        rect = QRectF(commit.rect)
        if commit.fill_mode != self.FILL_AROUND:
        ...
        if not outer.contains(point):
            return False
        return not rect.contains(point)

    def _reset_shape_commit_history(
        self, keep_circle: bool = False, keep_rectangle: bool = False
    ) -> None:
        if not keep_circle:
            self._active_circle_edit = None
            self._last_circle_commit = None
        if not keep_rectangle:
            self._active_rectangle_edit = None
            self._last_rectangle_commit = None

    def _begin_circle_edit(
        self, commit: _CircleCommit, drag_start: Optional[QPointF] = None
    ) -> None:
        self._active_circle_edit = self._clone_circle_commit(commit)
        center = QPointF(self._active_circle_edit.center)
        radius = float(self._active_circle_edit.radius)
        self._set_circle_preview((center, radius))
        if self._active_circle_edit.mask_base is not None or self._mask is not None:
            self._apply_circle(
                center,
                radius,
                fill_mode=self._active_circle_edit.fill_mode,
                thickness=self._active_circle_edit.thickness,
                record_history=False,
                base_mask=self._active_circle_edit.mask_base,
            )
        if drag_start is not None:
            self._shape_dragging = True
            self._shape_drag_last_pos = drag_start
        self._reset_shape_commit_history(keep_circle=True)

    def _begin_rectangle_edit(
        self, commit: _RectangleCommit, drag_start: Optional[QPointF] = None
    ) -> None:
        self._active_rectangle_edit = self._clone_rectangle_commit(commit)
        rect = QRectF(self._active_rectangle_edit.rect)
        self._set_rectangle_preview_rect(rect, record_history=False)
        if self._active_rectangle_edit.mask_base is not None or self._mask is not None:
            self._apply_rectangle_rect(
                rect,
                fill_mode=self._active_rectangle_edit.fill_mode,
                thickness=self._active_rectangle_edit.thickness,
                record_history=False,
                base_mask=self._active_rectangle_edit.mask_base,
            )
        if drag_start is not None:
            self._shape_dragging = True
            self._shape_drag_last_pos = drag_start
        self._reset_shape_commit_history(keep_rectangle=True)
    ...
    def _update_shape_preview(self, current: Optional[QPointF]) -> None:
        ...
        if rect.isNull() or rect.width() < 1 or rect.height() < 1:
            self._remove_shape_preview()
            return
        self._render_rectangle_preview(rect)

    def _set_rectangle_preview_rect(
        self, rect: Optional[QRectF], *, record_history: bool = False
    ) -> None:
        if rect is None:
            self._remove_shape_preview()
            return
        ...
        if rect.isNull() or rect.width() < 1 or rect.height() < 1:
            self._remove_shape_preview()
            return
        self._rectangle_preview_rect = rect
        self._render_rectangle_preview(rect)
        if record_history:
            base = self._apply_rectangle_rect(rect)
            if base is None and self._mask is None:
                return
            thickness = max(1, int(self._brush_radius))
            commit = _RectangleCommit(
                QRectF(rect),
                self._shape_fill_mode,
                thickness,
                base.copy() if base is not None else None,
            )
            self._active_rectangle_edit = commit
            self._last_rectangle_commit = self._clone_rectangle_commit(commit)
            self._reset_shape_commit_history(keep_rectangle=True)

    def _render_rectangle_preview(self, rect: QRectF) -> None:
        if rect.isNull() or rect.width() < 1 or rect.height() < 1:
            self._remove_shape_preview()
            return
        path = QPainterPath()
    ...
    def _update_circle_preview(self, current: Optional[QPointF]) -> None:
        if self._circle_preview is not None:
            center, radius = self._circle_preview
            self._render_circle_preview(center, radius)
            return
    ...
    def _set_circle_preview(self, circle: Optional[Tuple[QPointF, float]]) -> None:
        if circle is None:
            self._remove_shape_preview()
            return
        center, radius = circle
        if radius <= 0:
            self._remove_shape_preview()
            return
        self._circle_preview = (center, radius)
        self._render_circle_preview(center, radius)

    def _render_circle_preview(self, center: QPointF, radius: float) -> None:
        if radius <= 0:
            self._remove_shape_preview()
            return
        rect = QRectF(
    ...
    def _apply_rectangle(self, start: QPointF, end: QPointF) -> None:
        ...
        self._apply_rectangle_rect(rect)

    def _apply_rectangle_rect(
        self,
        rect: QRectF,
        *,
        ...
        fill_mode = fill_mode or self._shape_fill_mode
        draw_thickness = max(1, int(thickness if thickness is not None else self._brush_radius))

        if fill_mode == self.FILL_AROUND:
            outer_x0 = max(0, x0 - draw_thickness)
            outer_y0 = max(0, y0 - draw_thickness)
            outer_x1 = min(width - 1, x1 + draw_thickness)
            outer_y1 = min(height - 1, y1 + draw_thickness)
        ...
        if record_history:
            base_copy = self._mask.copy()
            self._push_undo()
            self._redo_stack.clear()
        else:
            base_copy = base_mask.copy() if base_mask is not None else np.zeros_like(self._mask)
            self._redo_stack.clear()

        self._mask = np.maximum(base_copy, shape_mask)
        self._update_mask_item()
        self.maskChanged.emit(self.mask())
        if record_history:
            self.historyChanged.emit()
    ...
    def _apply_three_point_circle(self) -> None:
        ...
        if circle is None:
            return
        center, radius = circle
        self._set_circle_preview((QPointF(center), float(radius)))
        base = self._apply_circle(center, radius)
        thickness = max(1, int(self._brush_radius))
        commit = _CircleCommit(
            QPointF(center),
            float(radius),
            self._shape_fill_mode,
            thickness,
        ...
        self._active_circle_edit = commit
        self._last_circle_commit = self._clone_circle_commit(commit)
        self._reset_shape_commit_history(keep_circle=True)
        self._circle_points.clear()
    ...
    def _move_rectangle_preview(self, scene_pos: QPointF) -> None:
        if self._rectangle_preview_rect is None or self._shape_drag_last_pos is None:
            return
        delta = scene_pos - self._shape_drag_last_pos
        if delta.isNull():
            return
        rect = self._rectangle_preview_rect
        new_left = rect.left() + delta.x()
        new_top = rect.top() + delta.y()
        scene_rect = self.scene_rect()
        if not scene_rect.isNull():
            max_left = scene_rect.right() - rect.width()
            max_top = scene_rect.bottom() - rect.height()
            if max_left < scene_rect.left():
                max_left = scene_rect.left()
            if max_top < scene_rect.top():
                max_top = scene_rect.top()
            new_left = min(max(new_left, scene_rect.left()), max_left)
            new_top = min(max(new_top, scene_rect.top()), max_top)
        new_rect = QRectF(new_left, new_top, rect.width(), rect.height())
        self._shape_drag_last_pos = scene_pos
        self._set_rectangle_preview_rect(new_rect, record_history=False)
        if self._active_rectangle_edit is not None:
            self._active_rectangle_edit.rect = QRectF(new_rect)
            self._apply_rectangle_rect(
                new_rect,
                fill_mode=self._active_rectangle_edit.fill_mode,
                thickness=self._active_rectangle_edit.thickness,
            ...
            self._last_rectangle_commit = self._clone_rectangle_commit(
                self._active_rectangle_edit
            )

    def _move_circle_preview(self, scene_pos: QPointF) -> None:
        if self._circle_preview is None or self._shape_drag_last_pos is None:
            return
        delta = scene_pos - self._shape_drag_last_pos
        if delta.isNull():
            return
        center, radius = self._circle_preview
        new_center = QPointF(center.x() + delta.x(), center.y() + delta.y())
        scene_rect = self.scene_rect()
        if not scene_rect.isNull():
            min_x = scene_rect.left() + radius
            max_x = scene_rect.right() - radius
            if max_x < min_x:
                min_x = max_x = scene_rect.center().x()
            min_y = scene_rect.top() + radius
            max_y = scene_rect.bottom() - radius
            if max_y < min_y:
                min_y = max_y = scene_rect.center().y()
            new_center.setX(min(max(new_center.x(), min_x), max_x))
            new_center.setY(min(max(new_center.y(), min_y), max_y))
        self._shape_drag_last_pos = scene_pos
        self._set_circle_preview((new_center, radius))
        if self._active_circle_edit is not None:
            self._active_circle_edit.center = QPointF(new_center)
            self._active_circle_edit.radius = float(radius)
            self._apply_circle(
                new_center,
                radius,
                fill_mode=self._active_circle_edit.fill_mode,
            ...
            self._last_circle_commit = self._clone_circle_commit(self._active_circle_edit)

    def _commit_rectangle_preview(self) -> None:
        if self._rectangle_preview_rect is None:
            return
        rect = self._rectangle_preview_rect
        if self._active_rectangle_edit is not None:
            self._active_rectangle_edit.rect = QRectF(rect)
            self._apply_rectangle_rect(
                rect,
                fill_mode=self._active_rectangle_edit.fill_mode,
                thickness=self._active_rectangle_edit.thickness,
        ...
        self._rectangle_preview_rect = None
        self._remove_shape_preview()
        if self._last_rectangle_commit is None:
            self._apply_rectangle_rect(rect)

    def _commit_circle_preview(self) -> None:
        if self._circle_preview is None:
            return
        circle = self._circle_preview
        center, radius = circle
        if self._active_circle_edit is not None:
            self._active_circle_edit.center = QPointF(center)
            self._active_circle_edit.radius = float(radius)
            self._apply_circle(
                center,
                radius,
            ...
            self._last_circle_commit = self._clone_circle_commit(
                self._active_circle_edit
            )
        self._circle_preview = None
        self._remove_shape_preview()
        if self._last_circle_commit is None:
            self._apply_circle(center, radius)
    ...
    def _apply_circle(
        ...
        *,
        ...
        if fill_mode == self.FILL_AROUND:
            outer_radius = radius_int + draw_thickness
...
class MaskEditor(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self._btn_circle.setToolTip(
            "Click three points to define a circle, drag to reposition, double-click to apply."
            " Click the circle again later to adjust it."
        )
        ...
        self._btn_rectangle.setToolTip(
            "Click and drag to draw a rectangle, drag to reposition, double-click to apply."
            " Click the rectangle again later to adjust it."
        )
